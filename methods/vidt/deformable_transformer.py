...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, tgt, query_pos, sketches=None):
        ...
        hs, inter_references = self.decoder(tgt, reference_points, memory,
                                            spatial_shapes, level_start_index, valid_ratios, query_pos, mask_flatten, sketches)
...
class CrossMHAFusion(nn.Module):
    ...
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        self.linear1_sk = nn.Linear(d_model, d_ffn)
        self.activation_sk1 = _get_activation_fn(activation)
        self.activation_sk2 = _get_activation_fn(activation)
        self.dropout3_sk = nn.Dropout(dropout)
        self.norm2_sk = nn.LayerNorm(d_ffn)
        self.linear2_sk = nn.Linear(d_ffn, d_model)
        self.dropout4_sk = nn.Dropout(dropout)
        self.norm3_sk = nn.LayerNorm(d_model)
        self.norm4_sk = nn.LayerNorm(d_model)

        self.dropout5_sk = nn.Dropout(dropout)
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
                query_pos=None, src_padding_mask=None, sketches=None):
        ...
        if self.return_intermediate:
            ...
            # sketches_return.append(sketches)
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if False:
                ...
                output, sket = self.query_fusion[lid](a,pos_img, tokens_sk, pos_sk)
                sketches, a_tgt = self.query_fusion_sketch[lid](tokens_sk, pos_sk, a , pos_img)
